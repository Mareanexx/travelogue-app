package ru.mareanexx.feature_profiles.presentation.screens.follows.viewmodel
...
import ru.mareanexx.network.domain.follows.usecase.FollowUserUseCase
import ru.mareanexx.network.domain.follows.usecase.GetFollowersUseCase
import ru.mareanexx.network.domain.follows.usecase.UnfollowUserUseCase
    ...
    private fun loadFollows() {
        viewModelScope.launch {
            getFollowersUseCase(_whoseProfileId)
                .catch { exception -> _uiState.value =
                    FollowsUiState.Error(exception.message ?: "Unknown error")
                }
                .collect { baseResult ->
                    when(baseResult) {
                        is BaseResult.Error -> { _eventFlow.emit(
                            FollowsEvent.ShowToast(baseResult.error)
                        ) }
