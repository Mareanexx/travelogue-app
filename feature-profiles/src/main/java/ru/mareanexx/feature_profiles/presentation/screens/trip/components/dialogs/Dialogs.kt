...
fun ErrorDialog(onRetry: () -> Unit, onNavigateBack: () -> Unit) {
    AlertDialog(
        ...
        title = { Text(stringResource(ru.mareanexx.feature_profiles.R.string.something_went_wrong), style = MaterialTheme.typography.titleSmall, color = primaryText) },
        text = { Text(stringResource(ru.mareanexx.feature_profiles.R.string.error_dialog_content), style = MaterialTheme.typography.labelMedium, color = primaryText) }
    )
}
...
fun ArrivalDatePickerDialog(
    ...
    DatePickerDialog(
        ...
        DatePicker(
            ...
            title = { Text(modifier = Modifier.padding(top = 10.dp, start = 20.dp), text = stringResource(
                ru.mareanexx.feature_profiles.R.string.select_arrival_date), style = MaterialTheme.typography.titleSmall) },
        )
    }
}