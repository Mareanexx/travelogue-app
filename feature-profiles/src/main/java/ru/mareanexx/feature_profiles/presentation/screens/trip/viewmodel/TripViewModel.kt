package ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel
...
import ru.mareanexx.feature_profiles.data.mappoint.mapper.toEditRequest
import ru.mareanexx.feature_profiles.data.mappoint.mapper.toForm
import ru.mareanexx.feature_profiles.data.mappoint.mapper.toNewRequest
import ru.mareanexx.feature_profiles.data.mappoint.remote.dto.MapPointWithPhotos
import ru.mareanexx.feature_profiles.data.pointphoto.mapper.toDeleted
...
import ru.mareanexx.feature_profiles.domain.mappoint.usecase.CreateMapPointUseCase
import ru.mareanexx.feature_profiles.domain.mappoint.usecase.DeleteMapPointUseCase
import ru.mareanexx.feature_profiles.domain.mappoint.usecase.EditMapPointUseCase
import ru.mareanexx.feature_profiles.domain.trip.usecase.GetTripWithMapPointsUseCase
import ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel.event.BottomSheetType
import ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel.event.DialogType
import ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel.event.TripEvent
...
import ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel.state.MapPointUiState
import ru.mareanexx.feature_profiles.presentation.screens.trip.viewmodel.state.TripUiState
import ru.mareanexx.network.domain.likes.usecase.AddLikeUseCase
import ru.mareanexx.network.domain.likes.usecase.DeleteLikeUseCase
...
class TripViewModel @Inject constructor(
    ...
    private fun getTripWithMapPoints() {
        viewModelScope.launch {
            getTripWithMapPointsUseCase(profileId, tripId)
                ...
                .catch { showTypifiedDialog(DialogType.Error) }
                .collect { result ->
                    when(result) {
    ...
    private suspend fun changeEditedMapPointBitmap(data: MapPointWithPhotos) {
        ...
        val imageUrl = data.photos.firstOrNull()?.filePath?.let { "${ApiConfig.apiFilesUrl}$it" }
        val bitmap = try {
            if (imageUrl != null) {
    ...
    suspend fun preloadMapPointBitmaps(context: Context) {
        ...
        newPoints?.forEach { pointWithPhotos ->
            val imageUrl = pointWithPhotos.photos.firstOrNull()?.filePath?.let { "${ApiConfig.apiFilesUrl}$it" }
